...
#if !defined(USE_WINAPI_CreateFileW)
	#define USE_WINAPI_CreateFileW 0
#endif
...
public:
	...
	void PerformRequest()
	{
		...
		if (bFailed)
		{
			...
			{
#if USE_WINAPI_CreateFileW
				DWORD  Access = GENERIC_READ;
				DWORD  WinFlags = FILE_SHARE_READ;
				DWORD  Create = OPEN_EXISTING;
				CreateFileW_EXTENDED_PARAMETERS Params = { 0 };
				Params.dwSize = sizeof(CreateFileW_EXTENDED_PARAMETERS);
				...
				HANDLE Handle = CreateFileW(GetFileNameForErrorMessagesAndPanicRetry(), Access, WinFlags, Create, &Params);
#else
